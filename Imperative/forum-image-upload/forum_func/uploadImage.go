...
func uploadImg(w http.ResponseWriter, r *http.Request) (string, string, error) {
	...
	if err != nil {
		return "false", "Error retrieving file from form-data", errors.New("")
	}
	defer file.Close()
	...
	if handler.Size > maxFileSize {
		return "false", "File too large to upload. Max size is 20mb", errors.New("file size exceeds 20mb")
	}
	// only allow jpeg, png and gif fileextentions
	ext := filepath.Ext(handler.Filename)
	if ext != ".jpeg" && ext != ".png" && ext != ".gif" && ext != ".jpg" {
		return "false", "File extention is not .jpeg, .png or .gif", errors.New("file extention not supported")
	}
	...
	if err != nil {
		return "false", "Error creating temp file on the server", err
	}
	defer tempFile.Close()

	fileBytes, err := ioutil.ReadAll(file)
	if err != nil {
		return "false", "Error reading bytes into file", err
	}
