...
func HandleCreate(w http.ResponseWriter, r *http.Request) {
	...
	case "POST": // POST = web server got request to generate something additional from user filled form
		...
		// topicImg, errorMsg, err := uploadImg(w, r)
		topicImg, _, _ := uploadImg(w, r)
		// if err != nil {
		// 	fmt.Println("e3", errorMsg)
		// 	fmt.Println("e2", err)
		// }
		...
		} else if len(strings.TrimSpace(topicTitle)) != 0 && len(strings.TrimSpace(topicText)) != 0 {
			if TopicType == "Topic" {
				if len(topicId) != 0 {
					if topicImg == "false" {
						topicImg = getImg(topicId)
					} else {
						deleteImg(topicId)
					}
