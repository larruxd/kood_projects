...
func MainTopics(databaseName, user, topics string) []ForumTopic {
	...
	sqlScript := `	select
					...
					left join (	select
								...
								from topic_content
								where status = 'ACTIVE') as content
...
func UpdateTopic(databaseName, user, id, name, text, img, removeImg string, categories []string) {
	img = maybeDeleteImg(id, removeImg, img)
...
func MyLikesDislikes(databaseName, user string) []MyLikeDislikeTopic {
	...
	sqlScript := `select distinct h.topic_id, h.topic_name, case when content_id is null then 'Main topic' else 'Comment' end, emotion
 	from topic_header h
	join topic_emotion e
		on h.topic_id = e.topic_id
	where h.status != 'DELETED' and e.user_email = '` + user + `' and e.status != 'DELETED'`
	value := SelectSQLScript(databaseName, sqlScript)
	var topic_name, topic_type, emotion_type sql.NullString
	var topic_id sql.NullInt16
	for value.Next() {
		value.Scan(&topic_id, &topic_name, &topic_type, &emotion_type)
		LikeValues := MyLikeDislikeTopic{
			TopicID:     int(topic_id.Int16),
			TopicName:   topic_name.String,
			TopicType:   topic_type.String,
			EmotionType: emotion_type.String}
...
func MyComments(databaseName, user string) []MyComment {
	...
	sqlScript := `select distinct h.topic_id, h.topic_name, c.content_name, c.content_text
 	from topic_header h
	join topic_content c
		on h.topic_id = c.topic_id
	where h.status != 'DELETED' and c.user_email = '` + user + `' and c.status != 'DELETED'`
	value := SelectSQLScript(databaseName, sqlScript)
	var topic_name, content_name, content_text sql.NullString
	var topic_id sql.NullInt16
	for value.Next() {
		value.Scan(&topic_id, &topic_name, &content_name, &content_text)
		Comments := MyComment{
			TopicID:     int(topic_id.Int16),
			TopicName:   topic_name.String,
			ContentName: content_name.String,
