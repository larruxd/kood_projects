...
function validateFile() {
    ...
    if (!allowedExtensions.exec(fileName)) {
        ...
        document.getElementById('fileErrorMsg').innerHTML = 'File size exceeds 20 MB limit.';
        fileInput.value = '';
        return false;
    }
    else {
...
function checkEmail(ctrl) {
    ...
    xhttp.onload = function () {
        if (this.responseText != "") {
            alert("Email already exists: " + this.responseText + ". Try another one!")
            ctrl.value = ""
        }
    }
    xhttp.open("POST", "/checkEmail");
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send("email=" + ctrl.value.trim());
}
...
//     if (!googleUser) {
//         console.log('Google user is null or undefined');
//         return;
//     }
...
function handleCredentialResponse(googleUser) {
    ...
    var email = JSON.parse(jsonPayload).email;
    var loginType = "google";
    var userName = JSON.parse(jsonPayload).given_name

    document.getElementById("hidden_email").value = email;
    document.getElementById("hidden_loginType").value = loginType;
    document.getElementById("hidden_user").value = userName;

    document.getElementById("form").submit(); //.getElementById("hidden-form")
