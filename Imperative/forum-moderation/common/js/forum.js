...
function showSubForm(id) {
    var elem = document.querySelector('[sub="' + id + '"]')
    if (elem != null) {
        document.getElementById("ContentMainId").value = elem.getAttribute("main")
        document.getElementById("TopicId").value = elem.getAttribute("sub")
    }
    if (typeof id == "undefined") {
        document.getElementById("ContentText").innerHTML = "";
    } else {
        document.getElementById("ContentText").innerHTML = document.getElementById("TopicComment" + id).innerHTML;
    }
    document.getElementById('NewSubTopic').classList.remove("hidden");
    const closeFormOnEscapekey = (event) => {
        if (event.key === "Escape") {
            document.getElementById('NewSubTopic').classList.add("hidden");
...
function likeMe(type, id, mainid, ctrl) {
    ...
    const element2 = document.getElementById('NewSubTopic');
