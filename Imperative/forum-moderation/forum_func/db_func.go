...
func MainTopics(databaseName, user, topics string) []ForumTopic {
	...
	sqlScript := `	select
						...
						content.ContentCnt,
						case when u.user_email = '` + user + `' or u1.user_type = 'ADMIN' then 1 else 0 end as flag,
					...
					where 
					('AllTopics' = '` + topics + `' and (h.status not in ('DELETED','DECLINE','PENDING') or (h.user_email = '` + user + `' and h.status != 'DELETED')))
						or ('MyTopics' = '` + topics + `' and h.user_email = '` + user + `' )
						or ('PendingTopics' = '` + topics + `' and h.status = 'PENDING' and h.user_email != '` + user + `')`
...
func UpdateTopic(databaseName, user, id, name, text, img, removeImg string, categories []string) {
	...
	sqlScript = `update topic_header set
					...
					status = 'PENDING' 
					where topic_id = ` + id + `
						and (user_email = '` + user + `' or (select user_type from users where user_email = '` + user + `') = 'ADMIN')`
...
func SubTopics(databaseName, user, id string) []ForumTopicContent {
	...
	sqlScript := `select 
							...
							case when c.user_email = '` + user + `' or u1.user_type = 'ADMIN' then 1 else 0 end as flag
					from topic_header h
...
func ReadReportsTable(databaseName, user string) []AdminReports {
	var values []AdminReports
	sqlScript := `	select r.report_id, u.user_name, r.question, r.answer 
					from report r 
					join users u
						on u.user_email = r.user_email
					where (r.user_email = '` + user + `') or (r.answer is null and (select user_type from users where user_email = '` + user + `') = 'ADMIN')`
	value := SelectSQLScript(databaseName, sqlScript)
	var email, question, answer sql.NullString
	var report_id sql.NullInt16
	for value.Next() {
		value.Scan(&report_id, &email, &question, &answer)
		reportValue := AdminReports{ReportID: int(report_id.Int16), Email: email.String, Question: question.String, Answer: answer.String}
		values = append(values, reportValue)
	}
	return values
...
func ReadSetupTable(databaseName, setup_type string) []string {
	var values []string
	sqlScript := `select setup_value from setup where setup_type = '` + setup_type + `' order by setup_value`
	value := SelectSQLScript(databaseName, sqlScript)
...
func ChangesAfterLastLogin(databaseName, user string) []ChangesAfterLogin {
	...
	sqlScript := `select distinct h.topic_id, h.topic_name, 'Topic changes' as type
	from topic_header h
	join users u
		on u.user_email = h.user_email
	where u.user_email = '` + user + `' and u.login_time_prev <= h.status_date

	union all

	select distinct h.topic_id, h.topic_name, 'New Comments on topic' as type
	from topic_header h
	join users u
		on u.user_email = h.user_email
	join topic_content c
	...
	where h.status != 'DELETED' and u.user_email = '` + user + `'
