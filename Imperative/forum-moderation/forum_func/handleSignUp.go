...
func HandleSignUp(w http.ResponseWriter, r *http.Request) {
	...
	case "POST": // POST = web server got request to generate something additional from user filled form
		...
		signin.LoginType = r.FormValue("typeselect")
		...
		if signin.LoginType != "USER" {
			sqlReportScript := "insert into report (user_email, question) values('" + signin.Email + "','Please add to me " + signin.LoginType + " user rights')"
			ExecSQLScript(DatabaseName, sqlReportScript)
		}
