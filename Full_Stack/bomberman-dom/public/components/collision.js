...
import { createNode } from '/framework/mini.js';
...
export function touchExplosion(moving) {
  ...
  const explosionImage = document.querySelectorAll(`.explosion`);
  //bomb cannot touch player if they are immune
  if (framework['players'][`${moving['myPlayerNum']}`].hasOwnProperty('immune') && framework['players'][`${moving['myPlayerNum']}`].immune)
    return undefined;
      ...
      switch (whichSideCollision(player, explosionImageRect)) {
        case 'bottom-right':
          return {
            ...
            playerKilledCoords: [Math.floor(moving.row), Math.floor(moving.col)],
          };
        case 'top-right':
          return {
            bomber: parseInt(explosionImage[i].classList.item(0).replace(/\D/g, '')),
...
export function propagateExplosion(rowChange, colChange, moving) {
  ...
  for (let r = 0; r < moving.flames; r++) {
    ...
    if (framework.cells[tmpMovingObj.row][tmpMovingObj.col] === globalSettings.wallTypes.softWall) {
...
export async function placeBombAndExplode(moving) {
  return new Promise((res) => {
    ...
    setTimeout(() => {
      bombElement.classList.replace(`player-${moving['myPlayerNum']}-bomb`, `player-${moving['myPlayerNum']}-explosion`);
